...
class App extends Component {
  state = {flagList: initialCountriesList}

  handleClickImageShowId = id => {
    const {flagList} = this.state
    const updatedList = flagList.map(eachCountry => {
      if (eachCountry.id === id) {
        return {...eachCountry, isVisited: !eachCountry.isVisited}
      }
      return eachCountry
    })
    this.setState({flagList: updatedList})
  }

  removeItemId = id => {
    this.setState(prevState => ({
      flagList: prevState.flagList.filter(item => item.id !== id),
    }))
  }

  render() {
    const {flagList} = this.state
    const showResults = flagList.filter(country => country.isVisited)
    return (
        ...
        <ul className="ul-button-container">
          {flagList.map(buton => (
            <ButtonDetails
              key={buton.id}
              buttonDetails={buton}
              handleClickImageShowId={this.handleClickImageShowId}
            />
          ))}
        </ul>
